...
const PanelHeading = ({ className, children }: PanelProps): ReactElement => {
  return (
    <div className="px-4 py-4 border-b border-gray-200 sm:px-6">
      <h3
        className={`text-2xl leading-6  text-gray-700 font-medium ${className}`}
      >
        {children}
      </h3>
    </div>
...
export default function Panel({
  ...
  return (
    <div
      id={id}
      className={`bg-white shadow rounded-md overflow-hidden  ${className}`}
    >
      {children}
    </div>
  );
